...
public class TERenderer {
    ...
    public void update(TETile[][] world, Doge d, char direction, boolean spotlight){
        if (spotlight){
            StdDraw.clear(Color.BLACK);
            for (int x = 0; x < world.length; x += 1) {
                for (int y = 0; y < world[0].length; y += 1) {
                    Point p = new Point(x, y);
                    if (!p.checkDistance(d)) {
        ...
        if (direction==' '){
            ...
            world[d.getX()][d.getY()].draw(d.getX() + xOffset, d.getY() + yOffset);
        }
        if (direction=='w'){
            world[d.getX()][d.getY()-1]=Tileset.FLOOR;
            world[d.getX()][d.getY()-1].draw(d.getX() + xOffset, d.getY()-1 + yOffset);
        }
        ...
        if (direction=='a'){
            world[d.getX()+1][d.getY()]=Tileset.FLOOR;
            world[d.getX()+1][d.getY()].draw(d.getX()+1 + xOffset, d.getY() + yOffset);
        }
        ...
        world[d.getX()][d.getY()]=d.getCharacter();
        world[d.getX()][d.getY()].draw(d.getX() + xOffset, d.getY() + yOffset);

        StdDraw.show();
    }
    public void singleTileChange(TETile[][] world, Point p, TETile t){
        world[p.getX()][p.getY()] = t;
        world[p.getX()][p.getY()].draw(p.getX() + xOffset, p.getY() + yOffset);
        StdDraw.show();
    }
    public static void updateNORENDER(TETile[][] world, Doge d, char direction){
        if (direction==' '){
            world[d.getX()][d.getY()]=Tileset.FLOOR;
        }
        if (direction=='w'){
